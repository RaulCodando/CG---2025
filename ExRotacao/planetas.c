...
void display(void){
    ...
    glPushMatrix();
        glRotatef ((GLfloat) -1*year, 0.0, 1.0, 0.0);
        glTranslatef (2.5, 0.0, 0.0); //Translada a partir do novo sistema de coordenadas resultante da Rotacao
        glRotatef ((GLfloat) day, 0.0, 1.0, 0.0);
        glColor3f(1.0, .0, .0);
        glutWireSphere(0.2, 10, 8);    /* desenha um planeta */

        glPushMatrix();
            glRotatef ((GLfloat) -1*year, 1.0, 1.0, 0.0);
            glTranslatef (1.0, 0.0, 0.0); //Translada a partir do novo sistema de coordenadas resultante da Rotacao
            glRotatef ((GLfloat) day, 0.0, 1.0, 0.0);
            glColor3f(1.0, 1.0, 1.0);
            glutWireSphere(0.1, 10, 8);    /* desenha uma lua */
        glPopMatrix();

        glPushMatrix();
            glRotatef ((GLfloat) -1*year, 1.0, 0.0, 0.0);
            glTranslatef (0.0, 1.0, 0.0); //Translada a partir do novo sistema de coordenadas resultante da Rotacao
            glRotatef ((GLfloat) day, 0.0, 1.0, 0.0);
            glColor3f(1.0, 1.0, 1.0);
            glutWireSphere(0.1, 10, 8);    /* desenha uma lua */
        glPopMatrix();
